...
const UserProfile: NextPage<UserProfileProps> = ({ user }) => {
    ...
    return (
        <>
            <Meta
                title={`${user?.name} (@${formatAccountName(user?.name!)}) | BuzzTime`}
                description={`${user?.name} on BuzzTime`}
                image={user?.image!}
            />
                                    ...
                                    user?.id !== session.data?.user?.id && (
                                        <div className="flex-shrink-0">
                                            <button
                                                ...
                                                className={`py-1 px-3 rounded text-sm mt-2 ${isCurrentlyFollowed ?? user?.followedByMe
